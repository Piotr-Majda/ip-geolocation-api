...
async def test_get_url_data_returns_data(app_service, repo, ip_data):
    ...
    # When
    result = await app_service.get_url_data("http://example.com")
    # Then
    assert result == ip_data
    repo.get_by_url.assert_awaited_once_with("http://example.com")
...
async def test_add_url_data_success(app_service, service, repo, ip_data):
    ...
    repo.exists_by_url = AsyncMock(return_value=False)
    # When
    result = await app_service.add_url_data("http://example.com")
    # Then
    assert result == ip_data
    service.get_geolocation_by_url.assert_awaited_once_with("http://example.com")
    repo.add.assert_awaited_once_with(ip_data)
...
async def test_add_url_data_not_found(app_service, service):
    ...
    # When/Then
    with pytest.raises(NotFoundGeolocationData):
        await app_service.add_url_data("http://example.com")
    service.get_geolocation_by_url.assert_awaited_once_with("http://example.com")
...
async def test_delete_url_data(app_service, repo):
    ...
    # When
    result = await app_service.delete_url_data("http://example.com")
    # Then
    assert result is True
    repo.delete_by_url.assert_awaited_once_with("http://example.com")
