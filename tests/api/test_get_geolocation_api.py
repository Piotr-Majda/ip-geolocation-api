...
async def test_get_geolocation_by_ip_address(
    ...
    """
    response = await test_client_v1.get(
...
@pytest.mark.asyncio
@pytest.mark.parametrize("ip_geolocation_data", test_data, indirect=True)
async def test_get_geolocation_by_url(
    test_client_v1, ip_geolocation_data, populate_db_with_ip_geolocation_data
):
    """
    Verify that the /geolocation endpoint returns a 200 status code and reports 'ok' status.
    """
    response = await test_client_v1.get("/geolocation/", params={"url": ip_geolocation_data.url})
    assert response.status_code == 200  # OK
    response_data = response.json()
...
@pytest.mark.asyncio
async def test_get_geolocation_by_ip_address_not_found(test_client_v1):
    """
    Should return 404 if IP not found.
    """
    response = await test_client_v1.get("/geolocation/", params={"ip_address": "8.8.8.8"})
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_get_geolocation_by_url_not_found(test_client_v1):
    """
    Should return 404 if URL not found.
    """
    response = await test_client_v1.get("/geolocation/", params={"url": "www.notfound.com"})
    assert response.status_code == 404


@pytest.mark.asyncio
async def test_get_geolocation_by_ip_and_url_422(test_client_v1):
    """
...
@pytest.mark.asyncio
async def test_get_geolocation_by_zero_input_422(test_client_v1):
    """
    Should return 422 if neither ip_address nor url is provided.
    """
    response = await test_client_v1.get("/geolocation/")
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_get_geolocation_by_ip_address_invalid_input(test_client_v1):
    """
    Should return 422 if ip_address is invalid.
    """
    response = await test_client_v1.get("/geolocation/", params={"ip_address": "999.999.999.999"})
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_get_geolocation_by_url_invalid_input(test_client_v1):
    """
    Should return 422 if url is invalid.
    """
    response = await test_client_v1.get("/geolocation/", params={"url": "not-a-url"})
    assert response.status_code == 422


@pytest.mark.asyncio
async def test_get_geolocation_by_url_invalid_input_without_suffix_on_domain(test_client_v1):
    """
    Should return 422 if url is invalid.
    """
    response = await test_client_v1.get("/geolocation/", params={"url": "http://www.ap"})
    assert response.status_code == 422


@pytest.mark.asyncio
@pytest.mark.parametrize("ip_geolocation_data", test_data, indirect=True)
async def test_get_geolocation_by_ip_address_database_down(
